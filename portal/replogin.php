...
if (isset ( $_POST ['login'] )) {
	if(! empty ( $_POST ['username'] ) && ! empty ( $_POST ['password'] )) {
		...
		$sql = "SELECT * FROM representative WHERE username='" . $_POST ['username'] . "' LIMIT 1;";
		...
		while ( $row = $result->fetch_assoc () ) {
			...
			if ($row ['password'] === md5 ( $_POST ['password'] )) {
				...
				$_SESSION ['adminflag'] = $row ['role'] === "admin" ? '1' : '0';
				$_SESSION ['customerflag'] = '0';
				$_SESSION ['supplierflag'] = $row ['role'] === "supplier" ? '1' : '0';
				$_SESSION ['deliveryflag'] = $row ['role'] === "delivery" ? '1' : '0';
...
?>
<div id="loginbox">
	<h3 style="background-color: rgba(255,255,255,0.8); padding: 0 5px;">Sales Login</h3>
	...
	<form id="loginform" class="form-vertical" action="replogin.php" method="post">
