          ...
          <div class="widget-content nopadding">
              ...
              <?php 
              	...
              	if ($delivery_flag == 1 || $supplier_flag == 1) {
              		...
              		echo "<th>NFF</th><th>Customer Name</th><th><i class='fa fa-phone'></i></th><th><i class='fa fa-calendar'></i></th><th>Date</th><th>Total Points</th>
					<th>Sales Price</th><th>Supplier Price</th><th>%margin</th><th>$ Type</th><th>Next Delivery Date</th><th>Open</th><th>Status</th><th>Sales Rep</th>";
              	}
              	if ($admin_flag == '1'){
              	...
              	while ($row = $result->fetch_assoc())
              	{
              		...
              		$sqlSupplier = "SELECT * FROM orders WHERE form_id=".$row['id'];
              		...
              		if ($row['edited_status'] == '1')
              		{
              			...
              			$sqlSupplier = "SELECT * FROM orders_updates WHERE form_id=".$row['id'];
              		}
              		...
              		$supprice = getSupplierPrice($conn, $sqlSupplier);
              		$percetage = (($price - $supprice) / $supprice) * 100;
              		
              		$percetage = number_format($percetage, 2, '.', '');
              		
              		$percetages = $percetages + $percetage;
              		...
              		if ($row['deliver3'] === '1') {
              			...
              			$confirmedImg = "<a href='customer.php?id=$row2[id]' class='expired-order-1'>3rd Delivered ($dd)</a>";
              		...
              		if ($delivery_flag == 1 || $supplier_flag == 1) {
              			...
              			<td>$row[signature_date]</td><td>$points</td><td style='background-color:#B4C5B8'>$ $price</td><td style='background-color:#f9b0ae'>$ $supprice</td>
              			<td style='background-color:#74b4d4'>$percetage %</td><td>$type</td><td><span id='$nextdateId'>$nextdate</span></td>
              			<td><a href='$modalId' data-toggle='modal'><span class='open-order'>OPEN</span></a></td>
              			<td>$confirmedImg</td>
              			<td><span id='rep$id' style='float: left'>$representative</span><span id='select$id' style='display:none'>$repSelect</span> $editRep</td>".
              			$delete;
              			echo "</tr>";
              		}
              	}
              	echo "<tr><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td>".number_format(((float)$percetages/(int)$cnt), 2, '.', '')." %</td>
