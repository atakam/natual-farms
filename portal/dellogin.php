...
if (isset ( $_POST ['login'] )) {
	if(! empty ( $_POST ['username'] ) && ! empty ( $_POST ['password'] )) {
		...
		$sql = "SELECT * FROM delivery_man WHERE username='" . $_POST ['username'] . "' LIMIT 1;";
...
?>
<div id="loginbox">
	<h3 style="background-color: rgba(255,255,255,0.8); padding: 0 5px;">Delivery Agent Login</h3>
	<select style="width: 100%" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
		...
		<option value="suplogin.php">Supplier</option>
		<option selected>Delivery Agent</option>
	</select>
	<span><?php echo $msg;?></span>
	<form id="loginform" class="form-vertical" action="dellogin.php" method="post">
